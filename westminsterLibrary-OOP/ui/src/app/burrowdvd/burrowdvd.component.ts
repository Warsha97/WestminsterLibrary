...
@Component({
  selector: 'app-burrowdvd',
  templateUrl: './burrowdvd.component.html',
  styleUrls: ['./burrowdvd.component.css']
})
export class BurrowdvdComponent implements OnInit {
...
errorStatus:string;
 ...
 showDVD(){
  this.appService.sendSelectedDVDData(this.isbnSelectedDVD).subscribe((data:any) => {
...
burrowDVD(){
  ...
  this.errorStatus = "OOPS! you can't complete this action. Either due to UNAVAILABILITY or you HAVEN'T SELECTED A DVD";
       ...
       this.appService.sendBurrowDVDData(this.isbnSelectedDVD,this.txtReaderId,this.year,this.month,this.day,this.hour,this.minute).subscribe((data:any) => {
